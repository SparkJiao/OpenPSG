...
from dataset import PSGTextDataset, collate_fn, PSGTextMultiTargetDataset
from evaluator import CLIPEvaluator
from models import CLIPConcat
from trainer import CLIPMulTarTrainer
...
parser.add_argument('--cosine_warmup', default=False, action='store_true')
...
savename = f'{args.model_name}_e{args.epoch}_lr{args.lr}_bs{args.batch_size}_wd{args.weight_decay}_wp{args.warmup_proportion}_' \
           f'{"cosine_lr_" if args.cosine_warmup else ""}' \
           f'prompt{args.add_prompt}_s{args.seed}_mul_tar'
...
# loading dataset
train_dataset = PSGTextMultiTargetDataset(stage='train', preprocess=preprocess, add_prompt=args.add_prompt)
...
# loading trainer
trainer = CLIPMulTarTrainer(model,
                            train_dataloader,
